	...
	return (
		<DropdownMenu>
			...
			<DropdownMenuContent align="end" className="w-56">
				{/* Switch active org */}
				<DropdownMenuItem disabled={isActive && hasActive} onSelect={(e) => e.preventDefault()}>
					<OrganizationSwitchButton
						organizationId={organizationId}
						trigger={
							<button type="submit" className="w-full flex items-center gap-2 text-left">
								<LucideArrowLeftRight className="w-4 h-4" />
								<span className={(!hasActive || isActive) ? "text-primary" : undefined}>
				...
				{isAdmin ? (
					<DropdownMenuItem asChild>
						<Link href={membershipsPath(organizationId)} className="flex items-center gap-2">
					...
					<Tooltip>
						...
						<TooltipContent><p>Admins only</p></TooltipContent>
					</Tooltip>
				)}
				...
				{isAdmin ? (
					<DropdownMenuItem onSelect={(e) => e.preventDefault()}>
						<OrganizationDeleteButton
							organizationId={organizationId}
							trigger={(isPending: boolean) => (
								<button type="button" className="w-full flex items-center gap-2 text-left text-red-600">
									{isPending ? (
										<LucideLoaderCircle className="w-4 h-4 animate-spin" />
									) : (
